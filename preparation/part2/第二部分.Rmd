...
## **带宽**
核密度估计无法消除z直方图中“密度函数受子区间宽度影响”的影响。

带宽：子区间的半径，定义了“在数据点附近领域的大小”，通常用h表示；  
窗宽：子区间的直径，通常用2h表示。  
...
如果带宽h越大，则数据点附近的领域越大，故偏差也越大(偏差与$h^2$成正比)；另一方面，如果带宽越大，则密度函数曲线越光滑，即方差越小(方差与$\frac{1}{nh}$成正比。
...
该模型的困难(与优点)在于$m(.)$是未知函数(连函数形式都未知)。因此我们只知道x与y之间存在回归关系，但是无法知道其具体的关系如何。  
